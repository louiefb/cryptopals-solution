...
def deflookup64():
    ...
    for letters in ascii_lowercase:
        lookup64.append([letters.upper(), i])
        i += 1
    for letters in ascii_lowercase:
        lookup64.append([letters, i])
        i += 1
    for j in range(0,10):
        lookup64.append([str(j), i])
        i += 1
    lookup64.append(["+", i])
    lookup64.append(["/", i+1])
deflookup64()

def frombase10(base, inputint):
    r = []
    if base == "ASCII":
        if type(inputint) != list:
            raise TypeError("inputint must be a list in order to process ASCII so there is separation")
        if [type(i) == int for i in inputint] == False:
            raise TypeError("inputint must be a list of integers")
        x = "".join([str(chr(i)) for i in inputint])
        return x
    if type(base) == int:
        x = inputint
        while x >= base:
            if base == 64:
                r.append(list(filter(lambda k: x%base in k, lookup64))[0][0])
            else:
                if x%base > 9:
                    r.append(list(filter(lambda k: x%base in k, lookup))[0][0])
                else:
                    r.append(str(x%base))
            x = x//base
        if base == 64:
            r.append(list(filter(lambda k: x in k, lookup64))[0][0])
        else:
            if x > 9:
                r.append(list(filter(lambda k: x in k, lookup))[0][0])
            else:
                r.append(str(x))
        r = [i for i in reversed(r)]
        return "".join(r)

def tobase10(base, inputint):
    if base == "ASCII":
        x = [ord(inputint[i]) for i in range(len(inputint))]
        return x
    if type(base) == int:
        x = 0
        j = len(str(inputint))-1
        for i in range(len(str(inputint))):
            if base == 64:
                x = x + list(filter(lambda k: str(inputint)[i] in k[0], lookup64))[0][1] * (base**j)
            else:
                if (str(inputint[i])).isnumeric() == True:
                    x = x + int(str(inputint)[i]) * (base**j)
                else:
                    x = x + list(filter(lambda k: str(inputint)[i] in k[0], lookup))[0][1] * (base**j)
            j -= 1
        return x

#issue here is a "0" is appended automatically leaving us with Ah == h == 33.
#quick solution could have been to allow a "0" appended to lefmost integer
input(tobase10(64, "Ah"))
input(tobase10(64, "h"))

def ASCIItobase64(inp):
    x = [frombase10(2, i) for i in tobase10("ASCII", inp)]
    ...
    while len(x)%6 != 0:
        x = x + "0"
    return frombase10(64, tobase10(2, x))

def base64toASCII(inp):
    x = [frombase10(2, tobase10(64, i)) for i in inp]
    for i in range(len(x)):
        while len(x[i])%6 != 0:
            x[i] = "0" + x[i]
    x = "".join(x)
    ...
    return frombase10("ASCII", [tobase10(2, i) for i in x])

def converter(basein, baseout, inp):
    if (basein == "ASCII") and (baseout == 64):
        x = ASCIItobase64(inp)
    elif (basein == 64) and (baseout == "ASCII"):
        x = base64toASCII(inp)
    else:
        if (type(basein) == int) and (type(baseout) == int):
            if type(inp) != str:
                raise TypeError("inp must be a string")
            if basein <= 16:
                inp = inp.upper()
            x = frombase10(baseout, tobase10(basein, inp))
            if baseout == 2:
                y = x
                x = []
                if basein == 64:
                ...
                if len(y)%size != 0:
                    while len(y)%size != 0:
                        y = "0" + y
                for i in range(len(y),0,-size):
                    x.insert(0, y[i-size:i])
        elif (basein == "ASCII") and (baseout == "ASCII"):
            x = inp
        if basein == "ASCII":
            if type(inp) != str:
                raise TypeError("inp must be a string")
            x = [frombase10(baseout, i) for i in tobase10(basein, inp)]
            if baseout == 2:
                y = []
                for i in x:
                    if len(i)%8 != 0:
            ...
            if type(inp) != list:
                raise TypeError("inp must be a list")
            if all(type(i) == int for i in inp):
                raise TypeError("inp must be a list of strings")
            x = [frombase10(baseout, [tobase10(basein, i)]) for i in inp]
    return x

def XOR(inp1, inp2, base1, base2, baseout):
    """performs XOR between two 'inp's. a single-byte inp2 would commence a single-byte XOR"""
    output = []
    x = converter(base1, 2, inp1)
    y = converter(base2, 2, inp2)
    if len(y) == 1: #assumes single-byte XOR
        while len(y) != len(x):
            y.insert(0,y[0])
    else:
        if len(y) < len(x):
    ...
    if len(x) == len(y):
        for i in range(len(x)):
            ...
            for j in range(len(x[i])):
                if (x[i][j] == "0" and y[i][j] == "1") or (x[i][j] == "1" and y[i][j] == "0"):
                    z.append("1")
                else:
                    z.append("0")
            output.append("".join(z))
    else:
        raise ValueError("the lengths of the base-2 of inp1 and inp2 do not match")
    if baseout == "ASCII":
        output = converter(2, baseout, output)
    else:
        if baseout != 2:
            output = [converter(2, baseout, i) for i in output]
        else:
            output = [converter(2, baseout, i)[0] for i in output]
    return output

def repeatingkeyXOR(inp, basein, baseout, *keys):
    """input 'inp' is XOR'd against ASCII keys"""
    x = converter(basein, 2, inp)
    y =[converter("ASCII", 2, i)[0] for i in keys]
    if len(x) < len(y):
        y = y[:len(x)]
    i = 0    
    while len(x) != len(y):
...
print(hammingdistance("this is a test", "wokka wokka!!!", "ASCII"))
print(hammingdistance("HUIfTQsPAh9PE048GmllH0kcDk4TAQsHThsBFkU2AB4BSWQgVB0dQzNTTmVS", "BgBHVBwNRU0HBAxTEjwMHghJGgkRTxRMIRpHKwAFHUdZEQQJAGQmB1MANxYG", 64))
input("pause")
...
def vigenere(inp):
    ...
    for i in text:
        for j in keysize:            
            for k in range(0,len(i)-j, j):
                #print(hammingdistance(i[k:j+k], i[j+k:j+k+j], 64))
                print(i, j, k, i[k:j+k], i[j+k:j+k+j], converter(64, 2, i[k:j+k]), converter(64, 2, i[j+k:j+k+j]), hammingdistance(i[k:j+k], i[j+k:j+k+j], 64))
                #input("")
                #print(hammingdistance(text[i:keysize], text[i, 64))
vigenere(text)


###############clue: converter(64, 2, "sP") = ['00001011', '00001111']
###############clue: converter(64, 2, "Ah") = ['00100001'] only :|
